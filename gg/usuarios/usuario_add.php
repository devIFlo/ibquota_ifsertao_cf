      ...
      <div class="container-fluid">
          <form action="<?php echo esc_url($_SERVER['PHP_SELF']); ?>" method="post">          
            ...
            <div class="form-group">
              <label for="exampleInputPassword">Matricula:</label>
              <input type="text" class="form-control" id="exampleInputPassword1" placeholder="Matricula" name="usuario">
              <label for="exampleInputPassword">Nome do Usu&aacute;rio:</label>
              <input type="text" class="form-control" id="exampleInputPassword1" placeholder="Nome do Usu&aacute;rio" name="nome_usuario">
            </div>
...
if (strlen($usuario)<1) {
   echo "<div class=\"alert alert-danger\" role=\"alert\">Campo Matricula n&atilde;o preenchido.</div><br><br><center>";
...
if (strlen($nome_usuario)<1) {
  echo "<div class=\"alert alert-danger\" role=\"alert\">Campo Nome do Usuario n&atilde;o preenchido.</div><br><br><center>";
  echo "<a class=\"btn btn-primary\" href=\"usuario_add.php\" role=\"button\" aria-expanded=\"false\">Voltar</a></center>";
  include '../includes/footer.php';
  exit(); 
}
...
if ($insert_stmt = $mysqli->prepare("INSERT INTO usuarios (cod_usuario, usuario, nome_usuario)
     VALUES (0, ?, ?)")) {
    $insert_stmt->bind_param('ss', $usuario, $nome_usuario);
    // Executar a tarefa pré-estabelecida.
    if (! $insert_stmt->execute()) {
      header('Location: error.php?err=Registration failure: INSERT Usuarios');
    }
    echo "<div class=\"alert alert-success\" role=\"alert\">Usuario <em>". $nome_usuario ."</em> Cadastrado com Sucesso.</div><br><br><center>";
